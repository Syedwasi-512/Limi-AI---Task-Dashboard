...
export const createTask = async (
  ...
  try {
    ...
    // taskController.ts mein createTask function ka ye part update karein:
    ...
    // 🔥 FIX: result.rows[0] check karein pehle
    if (result.rows && result.rows.length > 0) {
      const newTaskId = result.rows[0].id; // <--- Yahan [0] lazmi hai
