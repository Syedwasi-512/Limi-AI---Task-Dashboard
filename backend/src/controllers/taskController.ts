...
export const createTask = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (result.rows && result.rows.length > 0) {
      ...
      res.status(201).json(full.rows[0] || result.rows[0]);
    } else {
      res.status(500).json({ error: 'Database failed to return row' });
    }
  } catch (err) {
    console.error(err);
...
export const updateTask = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!full.rows || full.rows.length === 0) { res.status(404).json({ error: 'Task not found' }); return; }
